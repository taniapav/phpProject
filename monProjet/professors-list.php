...
require_once 'controller/professorsListCtrl.php';
          ...
          <tbody>
            ...
            <?php foreach ($usersList as $users) { ?>
              <tr>
                ...
                <td><?= $users->age ?></td>
                <td><a href="users-profile.php?id=<?php echo $users->id; ?>">Modifier Profil</a></td>
                <td><i data-toggle="modal" data-target="#deleteProfModal" data-id="<?= $users->id; ?>" data-lastname="<?= $users->lastname ?>" data-firstname="<?= $users->firstname ?>" class="fas fa-trash text-primary"></i></td>
              </tr>
            <?php } ?>
          </tbody>
        </table>
